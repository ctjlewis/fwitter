...
const deleteAllCollections = async (client) => {
  return client.query(
    Map(Paginate(Collections()), Lambda("ref", Delete(Var("ref"))))
  );
};

const deleteAllFunctions = async (client) => {
  return client.query(
    Map(Paginate(Functions()), Lambda("ref", Delete(Var("ref"))))
  );
};

const deleteAllRoles = async (client) => {
  return client.query(
    Map(Paginate(Roles()), Lambda("ref", Delete(Var("ref"))))
  );
};

const deleteIndexes = async (client) => {
  return client.query(
    Map(Paginate(Indexes()), Lambda("ref", Delete(Var("ref"))))
  );
};

const deleteTokens = async (client) => {
  return client.query(
    Map(Paginate(Documents(Tokens())), Lambda("ref", Delete(Var("ref"))))
  );
};
