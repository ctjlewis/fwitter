...
const main = async () => {
  ...
  if (typeof childDbName !== "undefined" && childDbName !== "") {
    ...
    const key = await handleSetupError(
      client.query(
        CreateKey({ database: Database(childDbName), role: "admin" })
      ),
      "Admin key - child db"
    );
    adminKey = key.secret;
  ...
  try {
    ...
    /** ****** CREATING FOR USER 1 ***********/
    await handleSetupError(
      faunaQueries.login("user1@test.com", "testtest"),
      "login user1"
    );
    const fw1 = await handleSetupError(
      faunaQueries.createFweet(
        "What do people do these days? #lockdown #corona #bored"
      ),
      "create fweet 1"
    );
    ...
    const fw2 = await handleSetupError(
      faunaQueries.createFweet(
        "My best friend is debt free, and I want to be famous #randomtweet "
      ),
      "create fweet 2"
    );
    ...
    await handleSetupError(
      faunaQueries.login("user2@test.com", "testtest"),
      "login user2"
    );
    const fw3 = await handleSetupError(
      faunaQueries.createFweet(
        "Fauna, the distributed database created by engineers that helped scale twitter"
      ),
      "create fweet 3"
    );
    const fw4 = await handleSetupError(
      faunaQueries.createFweet(
        `It's crazy how old the famous mascotte of the fwitter app has become`,
        {
          id: "old_Dino_osx4za",
          url:
            "https://res.cloudinary.com/dtkj34c6h/image/upload/v1584971195/old_Dino_osx4za.png",
          type: "image",
          cloudName: "dtkj34c6h",
        }
      ),
    ...
    await handleSetupError(
      faunaQueries.login("user3@test.com", "testtest"),
      "login user3"
    );
    const fw5 = await handleSetupError(
      faunaQueries.createFweet(
        "Someone give me me one reason to use FaunaDB instead MySQL #mysql #fauna"
      ),
      "create fweet 5"
    );
    const fw6 = await handleSetupError(
      faunaQueries.createFweet(
        "How to build an application like twitter with APIs like FaunaDB and Cloudinary!"
      ),
      "create fweet 6"
    );
    ...
    await handleSetupError(
      faunaQueries.login("user4@test.com", "testtest"),
      "login user 4"
    );
    const fw7 = await handleSetupError(
      faunaQueries.createFweet(
        "What do people generally do in time of crisis? Post pictures of cats #cats #lockdown #corona",
        {
          id: "cat_sugzem",
          url:
            "https://res.cloudinary.com/dtkj34c6h/video/upload/v1584971206/cat_sugzem.mp4",
          type: "video",
          cloudName: "dtkj34c6h",
        }
      ),
    ...
    await handleSetupError(
      faunaQueries.comment(fw2[0].fweet.ref, "ehh, interesting insight!"),
      "comment 1"
    );
    await handleSetupError(faunaQueries.likeFweet(fw1[0].fweet.ref), "like 1");
    const refw1 = await handleSetupError(
      faunaQueries.refweet(
        fw3[0].fweet.ref,
        `I've heard it's scalable, distributed, multi-region yet consistent?`
      ),
      "refweet 1"
    );
    ...
    await handleSetupError(
      faunaQueries.comment(fw7[0].fweet.ref, `Moar cats!`),
      "comment 3"
    );
    ...
    await handleSetupError(
      faunaQueries.comment(
        fw6[0].fweet.ref,
        `And an example using Auth0 for authentication`
      ),
      "comment 5"
    );
    await handleSetupError(
      faunaQueries.comment(refw1.refweet.fweet.ref, `Yes it is!`),
      "comment 6"
    );
    await handleSetupError(
      faunaQueries.comment(
        fw4[0].fweet.ref,
        `Wohah... time flies, even for dinosaurs`
      ),
      "comment 7"
    );
    ...
    await handleSetupError(
      faunaQueries.comment(fw4[0].fweet.ref, `Yep, it's crazy`),
      "comment 8"
    );
    await handleSetupError(
      faunaQueries.comment(fw7[0].fweet.ref, `Ooohhh.. cute!`),
      "comment 9"
    );
