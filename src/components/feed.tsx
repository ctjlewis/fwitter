...
const Feed = (props) => {
  ...
  const generateNotLoggedIn = () => {
    return (
      <div className="no-results-container">
        ...
        <img
          className="no-results-image"
          src="/images/dino-notloggedin.png"
          alt="no results"
        />
  ...
  const generateNotFound = () => {
    return (
      <div className="no-results-container">
        ...
        <img
          className="no-results-image"
          src="/images/dino-noresults.png"
          alt="no results"
        />
        <p className="no-results-subtext">
          You should be the very first to fweet
        </p>
      </div>
    );
  };
    ...
    faunaQueries
      ...
      .then((res) => {
        if (res.length > 0) {
          ...
          props.update(
            props.fweets.map((el) =>
              el.fweet.ref === fweetAndUser.fweet.ref ? newFweet : el
            ),
            true
          );
        } else {
  ...
  const handleRefweet = (fweetAndUser, message) => {
    faunaQueries
      ...
      .then((refweetAndOriginalUpdated) => {
        const newFweets = [refweetAndOriginalUpdated.refweet].concat(
          props.fweets
        );
        const originalIndex = newFweets.findIndex(
  ...
  return (
    <main className="body-content">
      <div className="fweets">
        {generateFweetsOrUserFeedback(
          props.fweets,
          props.error,
          props.loaded,
          user
