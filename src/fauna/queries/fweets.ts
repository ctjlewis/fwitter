  ...
  const FQLStatement = Let(
    {
      ...
      fweetWithUserAndAccount: GetFweetsWithUsersMapGetGeneric([
        Select(["ref"], Var("newFweet")),
      ]),
    },
    ...
    Do(
      ...
      If(
        ...
        Update(Select(["ref"], Get(Var("followerStatisticsRef"))), {
          data: {
            postlikes: Add(
              Select(["data", "postlikes"], Get(Var("followerStatisticsRef"))),
              Var("popularityGain")
            ),
          },
        }),
      ...
      Let(
        {
          ...
          updateOriginal: Update(fweetRef, {
            data: {
              refweets: Add(
                1,
                Select(["data", "refweets"], Var("originalFweet"))
              ),
            },
          }),
        ...
        },
        {
          ...
          refweet: Select(
            [0],
            GetFweetsWithUsersMapGetGeneric([Select(["ref"], Var("newFweet"))])
          ),
...
function createFweetWithoutUDF(client, message, tags) {
  return client
    .query(CreateFweet(message, tags))
...
function createFweet(client, message, asset) {
  ...
  return client
    .query(Call(q.Function("create_fweet"), message, hashTags, asset))
    .then((res) => flattenDataKeys(res));
}
...
function likeFweet(client, fweetRef) {
  return client
    .query(Call(q.Function("like_fweet"), fweetRef))
    .then((res) => flattenDataKeys(res));
}
...
function refweet(client, fweetRef, message) {
  const hashTags = findHashtags(message).map((t) => t.substring(1));
  return client
    .query(Call(q.Function("refweet"), fweetRef, message, hashTags))
    .then((res) => flattenDataKeys(res));
}
...
function comment(client, fweetRef, message) {
  return client
    .query(Call(q.Function("comment"), fweetRef, message))
    .then((res) => flattenDataKeys(res));
}
...
function GetFweetsWithUsersJoinExample2(client, email) {
  return client
    .query(
      Paginate(
        Join(
          Match(Index("accounts_by_email"), email),
          Index("fweets_by_author_simple")
        )
      )
    )
    ...
    q.Map(
      Paginate(
        Join(
          ...
          Lambda(
            ["fweetscore", "authorRef"],
            Match(Index("fweets_by_author"), Var("authorRef"))
          )
        )
      ),
...
function getFweets(client) {
  return client
    .query(Call(q.Function("get_fweets")))
    .then((res) => flattenDataKeys(res));
}
  ...
  const FQLStatement = Let(
    {
      ...
      tagReference: Select(
        [0],
        Paginate(Match(Index("hashtags_by_name"), tagName))
      ),
...
function getFweetsByTag(client, tag) {
  return client
    .query(Call(q.Function("get_fweets_by_tag"), tag))
    .then((res) => flattenDataKeys(res));
}
  ...
  const FQLStatement = Let(
    {
      ...
      authorReference: Select(
        [0],
        Paginate(Match(Index("users_by_alias"), authorAlias))
      ),
...
function getFweetsByAuthor(client, authorAlias) {
  return client
    .query(Call(q.Function("get_fweets_by_author"), authorAlias))
    .then((res) => flattenDataKeys(res));
}
      ...
      Let(
        {
            ...
            depth > 0
              ? Select(
                  ...
                  GetFweetsWithUsersMapGetGeneric(
                    [Select(["data", "original"], Var("fweet"))],
                    depth - 1
                  )
                )
              : false,
          ...
          fweetstats: If(
            Exists(Var("fweetstatsMatch")),
            Get(Var("fweetstatsMatch")),
            {}
          ),
