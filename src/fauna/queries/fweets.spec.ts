...
beforeAll(async () => {
  try {
    ...
    await handlePromiseError(
      registerWithUser(adminClient, "test@test.com", "testtest"),
      "Register with User"
    );
    const res = await handlePromiseError(
    ...
    await handlePromiseError(
      registerWithUser(adminClient, "test2@test.com", "testtest"),
      "Register with User"
    );
    const res2 = await handlePromiseError(
      login(adminClient, "test2@test.com", "testtest"),
      "Login"
    );
    ...
    await handlePromiseError(
      createFweet(loggedInClient, "Tweet user 1 #tag1", ["tag1"]),
      "Creating Fweet 1"
    );
    await handlePromiseError(
      createFweet(loggedInClient2, "Tweet user 2 #tag2 #tag3", [
        "tag2",
        "tag3",
      ]),
      "Creating Fweet 2"
...
it("A logged in user cant create tweets since it needs to use the UDF", function () {
  return expect(
    createFweetWithoutUDF(
      loggedInClient,
      "Today is a beautiful day to write another app #beautiful #day",
      ["beautiful", "day"]
    )
...
it("The initial client can only call the UDF functions, it cant access tweets", function () {
  return expect(getFweets(bootstrapClient)).rejects.toHaveProperty(
    ["message"],
    "permission denied"
  );
}, 60000);
