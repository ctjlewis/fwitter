...
async function setupDatabase(client) {
  ...
  await handleSetupError(
    createRateLimitingCollection(client),
    "profiles collection"
  );
  ...
  await handleSetupError(
    createFweetStatsCollection(client),
    "fweetstats collection"
  );
  await handleSetupError(
    createFollowerStatsCollection(client),
    "followerstats collection"
  );
  await handleSetupError(
  ...
  await handleSetupError(
    ...
    "function role - login"
  ); // without rate limiting: createFnRoleRegisterWithoutRateLimiting
  await handleSetupError(
    client.query(CreateFnRoleRegister),
    "function role - register"
  ); // without rate limiting: createFnRoleRegisterWithoutRateLimiting
  await handleSetupError(
    client.query(CreateFnRoleRegisterWithUser),
    "function role - register with user"
  ); // without rate limiting: createFnRoleRegisterWithoutRateLimiting
  await handleSetupError(
    client.query(CreateFnRoleManipulateFweet),
    "function role - create manipulate fweets"
  );
  ...
  await handleSetupError(
    client.query(CreateAccountWithUserUDF),
    "user defined function - register and create user"
  );
  await handleSetupError(
    client.query(CreateFweetUDF),
    "user defined function - create fweet (rate limited)"
  );
  await handleSetupError(
    client.query(LikeFweetUDF),
    "user defined function - like fweet"
  );
  await handleSetupError(
    client.query(RefweetUDF),
    "user defined function - refweet"
  );
  await handleSetupError(
    client.query(CommentUDF),
    "user defined function - comment"
  );
  await handleSetupError(
    client.query(GetFweetsUDF),
    "user defined function - get fweets"
  );
  await handleSetupError(
    client.query(GetFweetsByTagUDF),
    "user defined function - get fweets by tag"
  );
  await handleSetupError(
    client.query(GetFweetsByAuthorUDF),
    "user defined function - get fweets by user"
  );
  await handleSetupError(
    client.query(FollowUDF),
    "user defined function - follow"
  );
...
async function setupDatabaseAuthSpec(client) {
  ...
  await handleSetupError(
    createRateLimitingCollection(client),
    "Create Rate Limiting Collection"
  );
  await handleSetupError(
    createFollowerStatsCollection(client),
    "followerstats collection"
  ...
  await handleSetupError(
    client.query(CreateFnRoleLoginWithoutRateLimiting),
    "Create Login Fn role (no rate limiting)"
  );
  await handleSetupError(
  ...
  await handleSetupError(
    client.query(CreateLoginSimpleUDF),
    "Create Login UDF"
  );
  await handleSetupError(client.query(CreateAccountUDF), "Create Account UDF");
  await handleSetupError(
    client.query(CreateBootstrapRoleSimple),
    "Create Bootstrap Role"
  );
  await handleSetupError(
    client.query(CreatePowerlessRole),
    "Create Powerless Role"
  );
}
...
async function setupDatabaseSearchSpec(client) {
  ...
  await handleSetupError(
    createHashtagCollection(client),
...
async function setupProtectedResource(client) {
  ...
  await handleSetupError(
    client.query(
      If(
        Exists(Collection("something_protected")),
        true,
        CreateCollection({ name: "something_protected" })
      )
    ),
  ...
  await handleSetupError(
    client
      .query(
        Create(Collection("something_protected"), {
          data: { name: "mister-protected" },
        })
      )
      .then((res) => {
