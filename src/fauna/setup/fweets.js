...
async function createFweetsCollection(client) {
  await client.query(
    If(Exists(Collection("fweets")), true, CreateFweetsCollection)
  );
  await client.query(
    If(Exists(Index("all_fweets")), true, CreateIndexAllFweets)
  );
  await client.query(
    If(Exists(Index("fweets_by_author")), true, CreateIndexFweetsByAuthor)
  );
  await client.query(
    If(Exists(Index("fweets_by_tag")), true, CreateIndexFweetsByTag)
  );
  await client.query(
    If(Exists(Index("fweets_by_reference")), true, CreateIndexFweetsByReference)
  );
  await client.query(
    If(
      Exists(Index("fweets_by_hashtag_ref")),
      true,
      CreateIndexFweetsByHashtagRef
    )
  );
}
...
async function deleteFweetsCollection(client) {
  await client.query(
    If(Exists(Collection("fweets")), true, Delete(Collection("fweets")))
  );
  await client.query(
    If(Exists(Index("all_fweets")), true, Delete(Index("all_fweets")))
  );
  await client.query(
    If(
      Exists(Index("fweets_by_author")),
      true,
      Delete(Index("fweets_by_author"))
    )
  );
  await client.query(
    If(Exists(Index("fweets_by_tag")), true, Delete(Index("fweets_by_tag")))
  );
  await client.query(
    If(
      Exists(Index("fweets_by_reference")),
      true,
      Delete(Index("fweets_by_reference"))
    )
  );
  await client.query(
    If(
      Exists(Index("fweets_by_hashtag_ref")),
      true,
      Delete(Index("fweets_by_hashtag_ref"))
    )
  );
}
