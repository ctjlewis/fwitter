 ...
 * We use a wrapper helper to make sure that we override a function with
 * 'Update' in case it alread exists and Create it with 'CreateFunction' if it
 * did not exist yet. User Defined Functions (UDF):
 * 
 * https://docs.fauna.com/fauna/current/api/graphql/functions CreateFunction:
 * https://docs.fauna.com/fauna/current/api/fql/functions/createfunction Update:
 * https://docs.fauna.com/fauna/current/api/fql/functions/update
 */
const CreateAccountUDF = CreateOrUpdateFunction({
  name: "register",
  /**
   * Note that 'Lambda' requires two parameters to be provided when you call the
   * User Defined Function. The parameters will be bound to the variables
   * 'email' and 'password' which are used by the functions that we pass in.
   * Since these functions are in the scope of this lambda, they can access
   * these varaibles. (see above how these functions use Var('email) and
   * Var('password).
   * 
...
/**
 * Let's show a second example where the function immediately adds a user that
...
/**
 * Example of an identity based rate-limiting function Of course this requires
 * you to use UDFs since you do not want to give the end user access to:
...
/**
 * Another example of database logic where it makes sense to use an UDF. We keep
 * information in the 'fweetstats' about which user liked which post. 
 *
 * We also want to know the amount of likes on a post though which we could
 * calculate but it would probably not make sense to calculate it each time.
 * Instead we would save this count directly in the same transactions on the
 * 'fweet' entity.
 *
 * But how to secure this? Writing an ABAC rule to say that a user can only
 * increment or decrement the likes if it just liked the fweet (added to
 * fweetstas) is not feasible since it requires reasoning over updates on
 * multiple collections.
 *
 * Since we do not have a backend, the logical way to secure something like this
 * is a user defined function! That way, the user only has the rights to execute
 * this like_fweet function but it's not necessary to give him permissions to
 * directly write to the fweet.
 */
