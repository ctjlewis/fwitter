...
async function createFweetStatsCollection(client) {
  await client.query(
    If(Exists(Collection("fweetstats")), true, CreateFweetStatsCollection)
  );
  await client.query(
    If(
      Exists(Index("fweetstats_by_user_and_fweet")),
      true,
      CreateIndexFweetsStatsByUserAndFweet
    )
  );
}
...
async function deleteFweetStatsCollection(client) {
  await client.query(
    If(Exists(Collection("fweetstats")), true, Delete(Collection("fweetstats")))
  );
  await client.query(
    If(
      Exists(Index("fweetstats_by_user_and_fweet")),
      true,
      Delete(Index("fweetstats_by_user_and_fweet"))
    )
  );
}
...
const DeleteAllFweetStats = If(
  ...
  q.Map(
    Paginate(Documents(Collection("fweetstats"))),
...
export {
  DeleteAllFweetStats,
  createFweetStatsCollection,
  deleteFweetStatsCollection,
};
