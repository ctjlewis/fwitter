...
async function createHashtagCollection(client) {
  await client.query(
    If(Exists(Collection("hashtags")), true, CreateHashtagCollection)
  );
  await client.query(
    If(Exists(Index("hashtags_by_name")), true, CreateIndexHashtagByName)
  );
}

async function deleteHashtagCollection(client) {
  await client.query(
    If(Exists(Collection("hashtags")), true, Delete(Collection("hashtags")))
  );
  await client.query(
    If(
      Exists(Index("hashtags_by_name")),
      true,
      Delete(Index("hashtags_by_name"))
    )
  );
}
