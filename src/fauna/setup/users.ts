...
async function createUsersCollection(client) {
  ...
  await handlePromiseError(
    client.query(
      If(Exists(Index("users_by_account")), true, CreateUsersByAccount)
    ),
...
async function deleteUsersCollection(client) {
  await handlePromiseError(
    client.query(
      If(Exists(Collection("users")), true, Delete(Collection("users")))
    ),
    "Delete users collection"
  );
  await handlePromiseError(
    client.query(
      If(Exists(Index("users_by_alias")), true, Delete(Index("users_by_alias")))
    ),
    "Delete users_by_alias index"
  );
  await handlePromiseError(
    client.query(
      If(Exists(Index("all_users")), true, Delete(Index("all_users")))
    ),
  ...
  await handlePromiseError(
    client.query(
      If(
        Exists(Index("users_by_account")),
        true,
        Delete(Index("users_by_account"))
      )
    ),
