...
async function createAccountCollection(client) {
  await client.query(
    If(Exists(Collection("accounts")), true, CreateAccountsCollection)
  );
  await client.query(
...
async function deleteAccountsCollection(client) {
  await client.query(
    If(Exists(Collection("accounts")), true, Delete(Collection("accounts")))
  );
  await client.query(
    If(
  ...
  await client.query(
    If(Exists(Index("all_accounts")), true, Delete(Delete("all_accounts")))
  );
}
