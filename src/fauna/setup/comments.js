...
async function createCommentsCollection(client) {
  await client.query(
    If(Exists(Collection("comments")), true, CreateCommentsCollection)
  );
  await client.query(
    If(
      Exists(Index("comments_by_fweet_ordered")),
      true,
      CreateIndexCommentsByFweetOrdered
    )
  );
}
...
async function deleteCommentsCollection(client) {
  await client.query(
    If(Exists(Collection("comments")), true, Delete(Collection("comments")))
  );
  await client.query(
    If(
      Exists(Index("comments_by_fweet_ordered")),
      true,
      Delete(Index("comments_by_fweet_ordered"))
    )
  );
}
...
const DeleteAllComments = If(
  ...
  q.Map(
    Paginate(Documents(Collection("hashtags"))),
